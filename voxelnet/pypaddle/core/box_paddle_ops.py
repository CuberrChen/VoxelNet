...
import paddleplus
from paddleplus.tools import paddle_to_np_dtype
...
def paddle_box_encode(boxes, anchors, encode_angle_to_vector=False, smooth_axis=False):
...
def paddle_box_decode(box_encodings, anchors, encode_angle_to_vector=False, smooth_axis=False):
...
def lidar_to_camera(points, r_rect, velo2cam):
    num_points = points.shape[0]
    points = paddle.concat(
        [points, paddle.ones([num_points, 1]).astype(points)], axis=-1)
...
def multiclass_nms(nms_func,
    ...
    if not (boxes.shape[1] == scores.shape[1] or boxes.shape[1] == 1):
        raise ValueError('voxelnet dimension of boxes must be either 1 or equal '
                         'to the voxelnet dimension of scores')
